...
#### 1. Формат паспорта
Паспорта файлов имеют одинаковую длину (512 байт). Паспорт состоит из двух частей: постоянной (первые 64 байта паспорта) и специфической (остаток паспорта). Постоянная часть одинакова для всех типов файлов, специфическая же определяется типом данных, указанным в постоянной части.

##### Постоянная часть

| смещение | размер | тип (Си)         | описание                                                                 |
|----------|--------|------------------|--------------------------------------------------------------------------|
...
| 26       | 4      | unsigned long    | время начала приема (миллисекунд с начала дня)                          |
| 30-37    | 8      |                  | Резерв                                                                   |
| 38-59    | 22     |                  | Служебные                                                                |
| 60-61    | 2      |                  | Резерв                                                                   |
...
| Название спутника | Идентификатор |
|-------------------|---------------|
| NOAA-9            | 15427         |
| NOAA-10           | 16969         |
| NOAA-11           | 19531         |
...
|          |        |            | 1 - три 10 битовых слова                                                 |
|          |        |            | 2 - восемь 10 битовых слов                                               |
...
##### 2.2 Одноканальные данные NOAA AVHRR
| смещение | размер | тип          | описание                                                                 |
|----------|--------|--------------|--------------------------------------------------------------------------|
...
| 80       | 2      | int          | Максимальное значение среди значимых пикселов изображения (коэффициенты пересчета значения -> температура/альбедо) T = A * C + B |
...
#### Примечание 1.
До начала 2000 года поле `satName` описывалось как `char satName[15]`, а по смещению 16 располагалось поле `unsigned short satNum`. На тот момент в архиве были файлы данных только спутников NOAA (до NOAA-15 включительно), поэтому поле `satName` содержало ASCIIZ-строку "NOAA", а поле `satNum` - номер спутника в серии, т.е. для NOAA-12 оно содержало 12. В настоящий момент поле `satName` описывается как `char satName[13]` и для спутников NOAA содержит ASCIIZ-строку вида "NOAA 12", а по смещению 14 расположено поле `unsigned long satId`.
